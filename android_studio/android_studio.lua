...
function m.generate_run_configuration(wks)
	...
	config = config .. wks.runConfigurationModule
	...
	config = config .. 'activateToolWindowBeforeRun="false">'
  	p.x(config)
    p.x('<module name="android.' .. wks.runConfigurationModule .. '" />')
    
    if wks.runConfigurationOptions then
    	for name, value in pairs(wks.runConfigurationOptions) do
